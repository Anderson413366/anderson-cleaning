...
import { REDIRECT_MAP, RedirectConfig } from './lib/redirect-map'
...
async function checkRateLimitRedis(
  ...
  try {
    ...
    // If Redis fails, allow the request (fail open) but log error
    console.error('Redis rate limit error:', error)
    return { allowed: true, remaining: config.maxRequests, resetAt: now + config.windowMs }
  }
}

function checkRateLimitInMemory(request: NextRequest): {
  allowed: boolean
  resetAt?: number
  remaining?: number
} {
...
export async function middleware(request: NextRequest) {
  ...
  const rateLimit = await checkRateLimitRedis(request)

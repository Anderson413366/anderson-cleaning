...
export const TooltipProvider: React.FC<{ children: React.ReactNode; delayDuration?: number }> = ({ children, delayDuration = 100 }) => {
  ...
  const calculatePosition = () => {
    if (triggerRef?.current && tooltipRef.current) {
      ...
      if (top + tooltipRect.height > viewportHeight + window.scrollY) { // Check if goes off bottom
        top = triggerRect.top + window.scrollY - tooltipRect.height - 5; // Position on top
      }
       if (top < window.scrollY) { // Check if goes off top (when positioned above)
  ...
  return (
    <TooltipContext.Provider value={{ isOpen, setIsOpen, content, setContent, triggerRef, setTriggerRef }}>
      {children}
      <AnimatePresence>
        {isOpen && content && (
          <motion.div

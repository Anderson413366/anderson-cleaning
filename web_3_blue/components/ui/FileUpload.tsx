...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (!acceptedTypes.includes(file.type)) {
        setInternalError(t('fileTypeError', { types: acceptedTypes.map(t => t.split('/')[1].toUpperCase()).join(', ') }) as string);
        onChange(null);
  ...
  return (
    <div className="mb-6">
      ...
      <div className={`mt-1 flex flex-col items-center px-6 pt-5 pb-6 border-2 ${displayError ? 'border-red-400' : 'border-gray-300 dark:border-slate-600'} border-dashed rounded-md`}>
        <div className="space-y-1 text-center">
          ...
          <input ref={fileInputRef} id={name} name={name} type="file" className="sr-only" onChange={handleFileChange} accept={acceptedTypes.join(",")} />
        </div>
         <div className="mt-4 flex space-x-3">
            <Button type="button" onClick={triggerFileInput} variant="outline" size="sm">
              <EditIconCareers className="h-4 w-4 mr-2"/>
              {currentFile ? t('changeFileButton', {defaultValue: "Change File"}) as string : t('uploadFileButton') as string}
            </Button>
            {currentFile && (
      ...
      {displayError && <p className="mt-2 text-xs text-red-500 dark:text-red-400">{displayError}</p>}
    </div>
  );
};

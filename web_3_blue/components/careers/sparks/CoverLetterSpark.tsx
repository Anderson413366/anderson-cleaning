...
const CoverLetterSpark: React.FC<CoverLetterSparkProps> = ({ onSuggestion }) => {
  ...
  const handleGenerate = async () => {
    ...
    if (!position || experiences.length === 0) {
      setSparkResults(prev => ({ ...prev, [type]: t('coverLetterPromptDetailsMissing') as string}));
  ...
  return (
    <div className="mt-4 p-4 border border-dashed border-yellow-400 dark:border-yellow-600 rounded-lg bg-yellow-50 dark:bg-yellow-900/20">
      <div className="flex items-center mb-3">
        ...
        <h4 className="text-md font-semibold text-yellow-700 dark:text-yellow-300">{t('generateCoverLetterSparkButton') as string}</h4>
      </div>
      ...
      >
        {isLoadingSpark[type] ? t('generatingCoverLetterSpark') as string : (t('generateCoverLetterSparkButton') as string).replace("✨ ", "")}
      </Button>

      {isLoadingSpark[type] && <Spinner message={t('generatingCoverLetterSpark') as string} className="mt-3" />}

      {suggestionText && !isLoadingSpark[type] && (
        <div className="mt-4">
          <Alert 
            type={suggestionText.toLowerCase().includes("error") || suggestionText === t('coverLetterPromptDetailsMissing') as string ? "error" : "info"}
            title={suggestionText.toLowerCase().includes("error") || suggestionText === t('coverLetterPromptDetailsMissing') as string ? t("coverLetterSparkError") as string : t("coverLetterSparkSuggestion") as string}
            message={<p className="whitespace-pre-wrap">{suggestionText}</p>}
          />
          {!suggestionText.toLowerCase().includes("error") && suggestionText !== t('coverLetterPromptDetailsMissing') as string && (
            <Button onClick={() => onSuggestion(suggestionText)} variant="default" size="sm" className="mt-3">

...
const StrengthsSpark: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (!position || experiences.length === 0 || knowYouAnswers.length === 0) {
      setSparkResults(prev => ({ ...prev, [type]: [t('strengthsPromptDetailsMissing') as string] }));
  ...
  const hasError = strengths && strengths.length > 0 && (strengths[0].toLowerCase().includes("error") || strengths[0] === t('strengthsPromptDetailsMissing') as string);
  ...
  return (
    <div className="p-4 border border-dashed border-green-400 dark:border-green-600 rounded-lg bg-green-50 dark:bg-green-900/20">
      <div className="flex items-center mb-3">
        ...
        <h4 className="text-md font-semibold text-green-700 dark:text-green-300">{(t('strengthsSectionTitle') as string).replace("✨ ", "")}</h4>
      </div>
      <Button
      ...
      >
        {isLoadingSpark[type] ? t('generatingStrengths') as string : t('generateStrengthsButton') as string}
      </Button>

      {isLoadingSpark[type] && <Spinner message={t('generatingStrengths') as string} className="mt-3" />}
      ...
      {strengths && !isLoadingSpark[type] && (
        <div className="mt-4">
          {hasError ? (
             <Alert 
                type="error"
                title={t("strengthsError") as string}
                message={strengths[0]}
             />
          ) : (
            <>

...
const ReferencesSection: React.FC = () => {
  ...
  const getError = (index: number, field: keyof ReferenceEntry) => formErrors[`references.entries.${index}.${field}`];
    ...
    <SectionWrapper titleKey="referencesSectionTitle">
      {data.entries.map((entry, index) => (
        <div key={index} className="p-4 my-4 border border-gray-200 dark:border-slate-700 rounded-lg relative">
          <h4 className="text-md font-semibold mb-3 text-gray-700 dark:text-gray-300">{t('reference', {defaultValue: 'Reference'}) as string} #{index + 1}</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
            ...
            <FormInput
              ...
              onChange={(e) => handleChange('references', 'entries', e.target.value, index, 'relationship')}
            ...
            <FormInput
              ...
              type="tel"
              value={entry.phone}
              onChange={(e) => handleChange('references', 'entries', e.target.value, index, 'phone')}

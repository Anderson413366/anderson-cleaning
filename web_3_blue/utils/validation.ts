...
import { FormDataShape, SectionError } from '../types';
...
export const validateSectionData = (
  ...
  requiredFieldsPaths.forEach(fullPath => {
    ...
    } else if (typeof value === 'boolean' && value === false && (fieldName.endsWith('Ack') || fieldName === 'hasDriversLicense' && sectionData.applyingFor !== 'office_staff' /* Example conditional */)) {
    ...
    } else if (value === null && fieldName.startsWith('driversLicense')) { // File uploads
        errors[fullPath] = t('validationErrorMissingFile');
    } else if (Array.isArray(value) && value.length === 0 && (fieldName === 'entries' && (sectionId === 'workHistory' || sectionId === 'references'))) {
  ...
  if (sectionId === 'personalInfo') {
    if (sectionData.email && !EMAIL_REGEX.test(sectionData.email)) {
      errors['personalInfo.email'] = t('emailInvalidError', {defaultValue: 'Invalid email format.'});
    }
    if (sectionData.phone && !PHONE_REGEX.test(sectionData.phone)) {
      errors['personalInfo.phone'] = t('phoneInvalidError', {defaultValue: 'Invalid phone number format.'});
    }
  }
  
  if (sectionId === 'workHistory') {
    sectionData.entries?.forEach((entry: any, index: number) => {
      if (!entry.companyName?.trim()) errors[`workHistory.entries.${index}.companyName`] = t('requiredErrorText');
      if (!entry.position?.trim()) errors[`workHistory.entries.${index}.position`] = t('requiredErrorText');
      if (entry.fromDate && !DATE_MMYYYY_REGEX.test(entry.fromDate)) errors[`workHistory.entries.${index}.fromDate`] = t('dateInvalidError', {defaultValue: 'Invalid date (MM/YYYY).'});
      if (entry.toDate && !DATE_MMYYYY_REGEX.test(entry.toDate)) errors[`workHistory.entries.${index}.toDate`] = t('dateInvalidError', {defaultValue: 'Invalid date (MM/YYYY).'});
      if (entry.fromDate && entry.toDate && DATE_MMYYYY_REGEX.test(entry.fromDate) && DATE_MMYYYY_REGEX.test(entry.toDate)) {
        ...
        if (new Date(fromY, fromM -1) > new Date(toY, toM - 1)) {
           errors[`workHistory.entries.${index}.toDate`] = t('dateToBeforeFromError', {defaultValue: 'End date cannot be before start date.'});
        }
      }
    });
  }
  ...
  if (sectionId === 'references') {
     sectionData.entries?.forEach((entry: any, index: number) => {
      ...
      if (!entry.relationship?.trim()) errors[`references.entries.${index}.relationship`] = t('requiredErrorText');
      if (entry.phone && !PHONE_REGEX.test(entry.phone)) errors[`references.entries.${index}.phone`] = t('phoneInvalidError', {defaultValue: 'Invalid phone number format.'});
    });
  }
  
  if (sectionId === 'uploads') {
    if (!formData.uploads.driversLicense) { // Check from root formData for uploads

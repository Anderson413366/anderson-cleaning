...
const InterviewPrepSpark: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (!position || knowYouAnswers.length === 0) {
      setSparkResults(prev => ({ ...prev, [type]: [{ question: t('interviewPrepPromptDetailsMissing') as string, tip: '' }] }));
    ...
    try {
      ...
      setSparkResults(prev => ({ ...prev, [type]: [{ question: t('interviewPrepError') as string, tip: '' }] }));
    } finally {
      setIsLoadingSpark(prev => ({ ...prev, [type]: false }));
    }
  };
  ...
  const hasError = tips && tips.length > 0 && (tips[0].question.toLowerCase().includes("error") || tips[0].question === t('interviewPrepPromptDetailsMissing') as string);
  ...
  return (
    <div className="p-4 border border-dashed border-blue-400 dark:border-blue-600 rounded-lg bg-blue-50 dark:bg-blue-900/20">
       <div className="flex items-center mb-3">
        ...
        <h4 className="text-md font-semibold text-blue-700 dark:text-blue-300">{(t('interviewPrepSectionTitle') as string).replace("✨ ", "")}</h4>
      </div>
      <Button
      ...
      >
        {isLoadingSpark[type] ? t('generatingInterviewPrep') as string : t('generateInterviewPrepButton') as string}
      </Button>

      {isLoadingSpark[type] && <Spinner message={t('generatingInterviewPrep') as string} className="mt-3" />}

...
export const generateCoverLetterSuggestion = async (
  ...
  const experienceText = experiences.length > 0 ? `my experience in ${experiences.join(", ")}` : "my relevant skills";
...
export const identifyStrengths = async (
  ...
  const experienceText = experiences.length > 0 ? `Relevant experiences: ${experiences.join(", ")}.` : "No specific professional experiences listed.";
  const answersText = knowYouAnswers
    .filter(qa => qa.answer && qa.answer.trim() !== "")

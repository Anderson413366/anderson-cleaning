...
export interface Translations {
  [key: string]: string | { title: string; description: string }[] | { value: string; label: string }[] | string[];
}
...
export interface AppContextType {
  ...
  handleChange: (section: keyof FormDataShape, field: string, value: any, subIndex?: number, subField?: string) => void;
  handleMultiCheckboxChange: (section: keyof FormDataShape, field: string, option: string, checked: boolean) => void;
  ...
  setApplicationStatus: React.Dispatch<React.SetStateAction<'idle' | 'submitting' | 'success' | 'error'>>;
